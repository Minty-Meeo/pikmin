...
static void WriteMTXPS4x3(const Mtx mtx, volatile f32* dest)
{
	...
	asm("psq_l   %[tmp0], 0x00 (%[mtx]), 0, 0;"
	    ...
	    : [tmp0] "=g"(tmp0), [tmp1] "=g"(tmp1), [tmp2] "=g"(tmp2), [tmp3] "=g"(tmp3), [tmp4] "=g"(tmp4), [tmp5] "=g"(tmp5)
	    : [mtx] "r"(mtx), [dest] "r"(dest));
}

/**
 * @TODO: Documentation
 */
static void WriteMTXPS3x3from3x4(register const Mtx mtx, register volatile f32* dest)
{
	f32 tmp0, tmp1, tmp2, tmp3, tmp4, tmp5;
 ...
 */
static void WriteMTXPS3x3(register const Mtx33 mtx, register volatile f32* dest)
{
	...
	asm("psq_l  %[tmp0], 0x00(%[mtx]), 0, 0;"
	    ...
	    "psq_st %[tmp3],   0(%[dest]), 0, 0;"
	    "stfs   %[tmp4],   0(%[dest]);"
	    : [tmp0] "=g"(tmp0), [tmp1] "=g"(tmp1), [tmp2] "=g"(tmp2), [tmp3] "=g"(tmp3), [tmp4] "=g"(tmp4)
	    : [mtx] "r"(mtx), [dest] "r"(dest));
}
 ...
 * @TODO: Documentation
 */
static void WriteMTXPS4x2(register const Mtx mtx, register volatile f32* dest)
{
	...
	asm("psq_l  %[tmp0], 0x00 (%[mtx]), 0, 0;"
	    ...
	    "psq_st %[tmp2],   0 (%[dest]), 0, 0;"
	    "psq_st %[tmp3],   0 (%[dest]), 0, 0;"
	    : [tmp0] "=g"(tmp0), [tmp1] "=g"(tmp1), [tmp2] "=g"(tmp2), [tmp3] "=g"(tmp3)
	    : [mtx] "r"(mtx), [dest] "r"(dest));
}
